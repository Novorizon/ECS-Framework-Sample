#ifndef UNIVERSAL_LIT_HSB_INPUT_INCLUDED
#define UNIVERSAL_LIT_HSB_INPUT_INCLUDED
...
TEXTURE2D(_EmissionMapMask);    SAMPLER(sampler_EmissionMapMask);
...
half SampleEmissionMask(float2 uv, TEXTURE2D_PARAM(emissionMapMask, sampler_emissionMapMask))
{
    if (_Progress >= 0.75) {
        return 1;
    }

    half4 maskColor = SAMPLE_TEXTURE2D(emissionMapMask, sampler_emissionMapMask, uv);
    half mask = 1;
    if (_Progress >= 0.5) {
        mask = maskColor.b;
...
inline void InitializeStandardLitSurfaceData(float2 uv, out SurfaceData outSurfaceData)
{
    ...
    half emissionMask = SampleEmissionMask(uv, TEXTURE2D_ARGS(_EmissionMapMask, sampler_EmissionMapMask));
    outSurfaceData.emission *= emissionMask;
